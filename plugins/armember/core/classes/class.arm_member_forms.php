...
if (!class_exists('ARM_member_forms')) {
    ...
    margin-left: 10px;position: absolute;"></div>';
            ...
            if (is_user_logged_in()) {
                ...
                if ($form->exists() && !empty($form->fields)) {
                    ...
                    if($enable_crop){
                        ...
                        $content .='<button  class="arm_crop_cover_button" data_id="'.$formRandomID.'">' . __('crop', 'ARMember') . '</button>';
            ...
            if (empty($atts['id']) || $atts['id'] == 0 || (isset($_REQUEST['action']) && $_REQUEST['action'] == 'wpseo_filter_shortcodes')) {
                ...
                if ($form->exists() && !empty($form->fields)) {
                    ...
                    if ($form->type == 'registration') {
                        ...
                        foreach (array('facebook', 'twitter', 'linkedin', 'vk', 'insta') as $social_type) {
                ...
                if (!empty($form->fields)) {
                    ...
                    foreach ($form->fields as $field) {
                        ...
                        if ( ( isset($atts['type']) && $atts['type'] == 'edit_profile' ) || 'edit_profile' == $form->type ) {
                            if (in_array($field_options['type'], array('repeat_email'))) {
                                continue;
                            }
                    ...
                    if ($show_other_form_captcha_field == 1 || $arm_recaptcha_v3_status==1) {
                        ...
                        $field_content .='<input name="arm_settings_recaptcha_site_key" type="hidden" id="arm_settings_recaptcha_site_key"  value="' . ($arm_recaptcha_site_key) . '"  />';
                        $field_content .='<input name="arm_settings_recaptcha_theme" type="hidden" id="arm_settings_recaptcha_theme"  value="' . ($arm_recaptcha_theme) . '"  />';
            ...
            switch ($ffield_type) {
                    ...
                    if (is_admin() && isset($_GET['page']) && in_array($_GET['page'], array($arm_slugs->manage_members))) {
                        ...
                        if($enable_crop){
                            if ($value['meta_key'] == 'profile_cover') {
                                ...
                                $output .='<button  class="arm_crop_cover_button" data_id="'.$formRandomID.'">' . __('crop', 'ARMember') . '</button>';
                                ...
                                $output .='<button class="arm_crop_button" data_id="'.$formRandomID.'">' . __('crop', 'ARMember') . '</button>';
                ...
                case 'select':
                    ...
                    if (is_admin() && isset($_GET['page']) && in_array($_GET['page'], array($arm_slugs->manage_members))) {
                        ...
                        $output .= '<md-select class="' . esc_attr($class) . '" ' . $field_attr . $ng_change_func .'>';
                    ...
                    if (!empty($value['options'])) {
                        if (is_admin() && isset($_GET['page']) && in_array($_GET['page'], array($arm_slugs->manage_members))) {
                            ...
                            foreach ($value['options'] as $data) {
                                ...
                                $radio_controls .= '<md-radio-button '.$ngreadonly.' data-ng-value="\'' . esc_attr($key) . '\'" value="' . esc_attr($key) . '">' . esc_html($option) . '</md-radio-button>';
                    ...
                    if (!empty($value['options'])) {
                        ...
                        else 
                        {
                            ...
                            foreach ($value['options'] as $data) {
                                ...
                                $output .= '<md-checkbox '.$ngreadonly.' aria-label="' . esc_html($key) . '" ' . $chked . ' value="' . esc_attr($key) . '" data-ng-true-value="\'' . esc_html($key) . '\'" data-ng-false-value="null" class="' . esc_attr($class) . '" ' . $field_attr . ' id="' . $ngModelCheck . '">';
        ...
        function arm_admin_save_member_details($member_data = array()) {
            global $wp, $wpdb, $current_user, $arm_slugs, $arm_errors, $ARMember, $arm_members_class, $arm_global_settings, $arm_subscription_plans, $arm_buddypress_feature, $arm_manage_communication, $is_multiple_membership_feature, $arm_pay_per_post_feature, $arm_subscription_cancel_msg;
			...
			$common_messages = isset($arm_global_settings->common_message) ? $arm_global_settings->common_message : array();
                    ...
                    if ($planObj->exists() && $planObj->is_recurring()) {
                        do_action('arm_cancel_subscription_gateway_action', $user_ID, $arm_plan_id);
                    }
        ...
        function arm_member_update_meta_details($user_ID, $posted_data = array()) {
            global $wp, $wpdb, $current_user, $arm_errors, $ARMember, $arm_subscription_plans, $payment_done, $arm_members_class, $is_multiple_membership_feature, $arm_pay_per_post_feature;
            $arm_errors = new WP_Error();
